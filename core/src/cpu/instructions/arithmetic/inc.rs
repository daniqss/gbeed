...
pub struct Inc<D: WritableOperand> {
    dst: D,
}
impl<D: WritableOperand> Inc<D> {
    pub fn new(dst: D) -> Self { Self { dst } }
}
impl<D: WritableOperand> Instruction for Inc<D> {
...
#[derive(Debug, Default, Clone, Copy)]
pub struct Inc16<D: Operand16> {
    dst: D,
}
impl<D: Operand16> Inc16<D> {
    pub fn new(dst: D) -> Self { Self { dst } }
}
impl<D: Operand16> Instruction for Inc16<D> {
    fn exec(&mut self, gb: &mut Dmg) -> InstructionResult {
        let result = self.dst.load(gb).wrapping_add(1);
