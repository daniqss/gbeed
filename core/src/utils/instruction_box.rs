...
/// # Instruction Box
/// Container for instructions that allows to store them in a single type
/// without heap allocation and with a vtable for dynamic dispatch.
...
impl InstructionBox {
    pub fn new<T: Instruction + Copy + 'static>(val: T) -> Self {
        ...
        let vtable = unsafe {
            let (_data_ptr, vtable_ptr): (*const (), *const ()) =
                core::mem::transmute::<&dyn Instruction, _>(&val as &dyn Instruction);
            vtable_ptr
        };
    ...
    fn as_dyn_ptr(&self) -> *const dyn Instruction {
        unsafe { core::mem::transmute((self.data.as_ptr() as *const (), self.vtable)) }
    }

    #[inline]
    fn as_dyn_ptr_mut(&mut self) -> *mut dyn Instruction {
        unsafe { core::mem::transmute((self.data.as_mut_ptr() as *const (), self.vtable)) }
    }
}
...
impl core::ops::DerefMut for InstructionBox {
    #[inline]
