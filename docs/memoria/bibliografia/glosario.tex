%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% Obxectivo: Lista de termos empregados no documento,                          %
%            xunto cos seus respectivos significados.                          %
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\newglossaryentry{bytecode}{
  name=bytecode,
  description={Código independente da máquina que xeran compiladores de determinadas linguaxes (Java, Erlang,\dots) e que é executado polo correspondente intérprete}
}

\newglossaryentry{homebrew}{
  name=homebrew,
  description={Software desenvolvido de xeito amateur pola comunidade para unha plataforma, á marxe das canles oficiais do seu fabricante}
}

\newglossaryentry{nibble}{
  name=nibble,
  description={Grupo de catro bits, é dicir, media palabra de oito bits. Un byte divídese no nibble alto (bits 4--7) e no nibble baixo (bits 0--3), cada un representable cun único díxito hexadecimal}
}

\newglossaryentry{opcode}{
  name=opcode,
  description={Primeiro byte dunha instrución, que identifica a operación que debe realizar o procesador e, no caso do SM83, codifica tamén os operandos sobre os que actúa}
}

\newglossaryentry{modding}{
  name=modding,
  description={Práctica de modificar o hardware orixinal dunha consola, xeralmente para mellorar as súas prestacións ou adaptalo aos estándares actuais, como substituír a pantalla ou engadir unha batería recargable}
}

\newglossaryentry{tile}{
  name=tile,
  plural=tiles,
  description={Bloque de 8$\times$8 píxeles almacenado na \acrshort{vram} en formato \textit{2bpp}, no que cada píxel se codifica con dous bits}
}

\newglossaryentry{viewport}{
  name=viewport,
  description={Área rectangular da pantalla actualmente visible dun programa gráfico}
}

\newglossaryentry{magicValues}{
  name=valor máxico,
  plural=valores máxicos,
  description={Cadena de texto ou número cun significado especial, non obvio sen coñecemento adicional do programa}
}