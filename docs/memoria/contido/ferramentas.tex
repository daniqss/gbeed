...
\lettrine{N}{este} capítulo fálase das diferentes tecnoloxías e ferramentas empregadas para o desenvolvemento do traballo:
a linguaxe de programación e o seu ecosistema, a librería gráfica empregada nas interfaces
e as ferramentas que sosteñen o fluxo de traballo do proxecto.
Péchase o capítulo cunha xustificación da elección destas tecnoloxías
...
con soporte tanto para o despache estático, resolto por monomorfización,
...
As dependencias decláranse no manifesto \texttt{Cargo.toml}
...
\textit{rustfmt} e \textit{clippy} son o formatador e o \textit{linter} oficiais, para manter un código limpo e con estilo consistente.
Neste proxecto tamén se usa \textit{cargo-flamegraph}, que permite dende a interface de Cargo xerar gráficos
a partir dos datos obtidos empregando o perfilador \textit{perf} de Linux.

\section{Raylib como librería gráfica}
Raylib~\cite{Raylib}
é unha librería escrita en C99 e creada por Ramon Santamaría (Raysan5) para crear videoxogos de forma sinxela.
...
en GLFW~\cite{Glfw},
unha librería multiplataforma compatible con Windows, macOS e Linux,
tanto baixo X11 como baixo Wayland.
Na web mantense a mesma interface:
o compilador Emscripten proporciona unha implementación da \acrshort{api} de GLFW sobre o navegador,
...
En sistemas embebidos GLFW substitúese por completo empregando osubsistema \acrshort{drm} de Linux,
sen necesidade dun servidor gráfico completo.
...
para a Raspberry Pi empregada como plataforma de referencia, co emulador e o soporte da pantalla xa integrados.
Deste xeito, non só a compilación senón tamén a despregadura no hardware final é reproducible.
...
Os binarios para \texttt{armv6l} constrúense dentro dun contedor,
definido nun \textit{Dockerfile} e invocado dende receitas de just
...
A xeración de código axústase ao procesador do dispositivo,
e o \textit{Dockerfile} copia primeiro os manifestos de cargo
...
A compilación cruzada realízase coa
...
Ao executarse o compilador de forma nativa a compilación é rápida,
e o binario resultante, enlazado con glibc e co backend X11 de raylib,
...
que compila a interface e copia os artefactos a un directorio de distribución
...
\section{Hardware empregado}
Co obxectivo de crear unha consola portátil de referencia empregando o emulador para demostrar as súas capacidades,
escolleuse hardware de baixo custo, amplamente empregado e documentado e de doada adquisición,
para que calquera persoa poida reproducir a construción sen recorrer a compoñentes descontinuados ou a montaxes a medida,

A este criterio engádese un segundo requisito, que o dispositivo poida executar un sistema operativo Linux
de propósito xeral en lugar de firmware \textit{bare metal},
de forma que a consola non quede limitada ao emulador
e o usuario conserve a capacidade de ampliar o uso que lle dá ao dispositivo.

\subsection{Raspberry Pi Zero}
Como microcomputador empregouse a familia de \acrshort{sbc} Raspberry Pi Zero~\cite{RaspberryPiZero},
probablemente a opción máis coñecida e documentada dentro deste rango de prezo.
Durante o desenvolvemento empregáronse dous modelos, a Raspberry Pi Zero W e a Raspberry Pi Zero 2~W.
Desta forma valídase o emulador en dúas arquitecturas distintas:
a primeira conta cun único núcleo \texttt{armv6l},
mentres que a segunda incorpora catro núcleos Cortex-A53 de 64 bits,
capaces de executar tanto código \texttt{aarch64} como código de 32 bits.
O usuario pode escoller que modelo empregar, segundo as súas necesidades.

En ambos casos empregouse a variante \textbf{W} durante o desenvolvemento, que engade conectividade sen fíos.
Esta non é necesaria para o funcionamento da consola,
pero si mellora notablemente a experiencia de desenvolvemento,
xa que permite acceder ao dispositivo por rede para copiar binarios e depurar a execución
sen ter que extraer a tarxeta SD en cada iteración.
A nivel de usuario final o comportamento é idéntico ao dos modelos sen conectividade.

\subsection{GamePi13}

Para lograr unha experiencia de consola portátil completa, empregouse o GamePi13~\cite{GamePi13}.
Este dispositivo consiste dunha placa que se conecta directamente aos \acrshort{gpio} da Raspberry Pi Zero sen necesidade de soldadura,
e inclue un controlador ST7789 para a súa pantalla \acrshort{spi}, un altofalante, unha saída de auriculares
e un conxunto de botóns coa disposición habitual dunha consola portátil, cos que se recolle a entrada do usuario.
...
e un proceso de desenvolvemento e despregadura reproducible en todas esas plataformas.
...
cunha \acrshort{api} máis sinxela e compatible cunha grande cantidade de plataformas.

En calquera caso, esta dependencia queda confinada nas interfaces de usuario:
o núcleo do emulador non depende de raylib.
Deste xeito, crear outra interface de usuario empregando outra librería para outro contexto diferente
é trivial. O desenvolvedor só tería que implementar as interfaces que demanda o emulador
e xa podería adecuar o resto da interface de usuario ás súas necesidades.
