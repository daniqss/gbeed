    ...
    fn exec(&mut self) -> InstructionResult {
        let (dst, src, address, cycles, len): (&mut u8, u8, Option<u16>, u8, u8) =
            match (&mut self.dst, &self.src) {
                ...
                (ID::PointedByN16(bus, address), IT::Reg8(src, reg)) if *reg == R8::A => {
                ...
                (ID::PointedByCPlusFF00(bus, address), IT::Reg8(src, reg)) if *reg == R8::A => {
                ...
                (ID::Reg8(dst, reg), IT::PointedByN16(src, address)) if *reg == R8::A => {
                    (dst, *src, Some(*address), 3, 2)
                }
                // copy the src byte addressed by 0xFF00 + C into dst register A
                (ID::Reg8(dst, reg), IT::PointedByCPlusFF00(src, address)) if *reg == R8::A => {
