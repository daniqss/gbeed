...
impl Cpu {
    ...
    pub fn fetch(gb: &mut Dmg, opcode: u8) -> FetchResult {
        ...
        let instruction: Box<dyn Instruction> = match opcode {
            ...
            0x20 => Jr::new(IT::JumpToImm8(JC::NotZero(cpu.not_zero()), gb[cpu.pc + 1] as i8)),
            ...
            0x27 => Daa::new(),
            0x28 => Jr::new(IT::JumpToImm8(JC::Zero(cpu.zero()), gb[cpu.pc + 1] as i8)),
            ...
            0x37 => Scf::new(),
            0x38 => Jr::new(IT::JumpToImm8(JC::Carry(cpu.carry()), gb[cpu.pc + 1] as i8)),
